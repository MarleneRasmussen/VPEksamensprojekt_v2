    ...
    public void drawItems(Graphics2D g2d) {
        for (ItemObject item : items) {
            if (item.getLocationNumber() == DungeonMap.getCurrentWorldLocation()) {
                /*if (item.getItemProperties() == ItemProperties.DOOR_VERTICAL) {
                    g2d.drawImage(item.getImage(), item.getPosX(), item.getPosY(), Config.CELL_SIZE * 2, Config.CELL_SIZE, null);
                } else if (item.getItemProperties() == ItemProperties.DOOR_HORIZONTAL) {
                    g2d.drawImage(item.getImage(), item.getPosX(), item.getPosY(), Config.CELL_SIZE, Config.CELL_SIZE * 2, null);
                } else {*/
